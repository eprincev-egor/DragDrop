    ...
    $(document).on("mouseup touchend", function(e) {
        ...
        if ( activeDragDrops.length ) {
            ...
            for (id in objs) {
                dragDrop = objs[id];
                if ( 
                    "drop" in dragDrop._events && 
                    activeDragDrops.indexOf(dragDrop) == -1 
                 ) {
                    var $elem;
                    for (var i=0, n=dragDrop._elems.length; i<n; i++) {
                        $elem = dragDrop._elems[i];
                        
                        if ( $elem[0] == target || $elem.hasChild($target) ) {
                            var dropEvent = dragDrop.createDropEvent(e, $elem);
                            
                            for (var j=0, m=activeDragDrops.length; j<m; j++) {
                                activeDragDrops[j].decorateDrop(dropEvent, $elem);
                            }
                            
                            dragDrop.dragout(e, $elem);
                            dragDrop.drop(dropEvent, $elem);
                            hasDrop = true;
                        }
                    }
                }
            ...
            if ( hasDrop ) {
                activeDragDrops.forEach(function(dragDrop) {
                    dragDrop._activeElems.forEach(function($elem) {
                        dragDrop.dropEnd(e, $elem);
                    });
                });
            }
        }
